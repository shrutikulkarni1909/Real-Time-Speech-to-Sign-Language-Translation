...
from backend.gloss.dataset_loader import load_dataset_for_training, load_daily_pairs
from backend.gloss.model import Seq2Seq
from backend.gloss.tokenizer import SimpleTokenizer
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Seq2Seq text->gloss model.")
    parser.add_argument(
        "--csv-path",
        default="backend/datasets/aslg_pc12/train.csv",
        help="Path to ASLG_PC12 train.csv with text,gloss columns",
    )
    parser.add_argument("--epochs", type=int, default=2)
    parser.add_argument("--batch-size", type=int, default=8)
    args = parser.parse_args()

    train_model(args.csv_path, epochs=args.epochs, batch_size=args.batch_size)
